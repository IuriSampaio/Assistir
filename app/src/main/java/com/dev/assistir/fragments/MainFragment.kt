...
import com.dev.assistir.activitys.BrowseErrorActivity
...
class MainFragment : BrowseFragment() {
    ...
    private fun loadRows(MoviesListByGenre: List<List<Filme>>) {
        // ADAPTADOR DE LINHAS
        val rowsAdapter = ArrayObjectAdapter(ListRowPresenter())

        // TITULO DO GRID MENU
        val gridHeader = HeaderItem(20.toLong(), "Assistir ...")
        // CRIANDO ARRAY ADAPTER PARA OS ITENS DO MENU
        val gridRowAdapter = ArrayObjectAdapter(GridItemPresenter())
            gridRowAdapter.add("Movie")
            gridRowAdapter.add("Series")
            gridRowAdapter.add("LiveTv")

        // ADICIONANDO GRID MENU AO ADAPTADOR DE LINHAS
        rowsAdapter.add(ListRow(gridHeader, gridRowAdapter))

        MoviesListByGenre.map{MoviesList ->
            val index = MoviesListByGenre.indexOf(MoviesList)
            ...
            val header = HeaderItem(index.toLong(), FilmeList.MOVIE_CATEGORY[index])
    ...
    private inner class ItemViewSelectedListener : OnItemViewSelectedListener {
        override fun onItemSelected(itemViewHolder: Presenter.ViewHolder?, item: Any?,
            ...
            if (item is Movie) {
                ...
                mBackgroundUri = item.backdrop_path
                startBackgroundTimer()
            }
        }
    }
